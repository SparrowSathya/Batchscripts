...
if exist "%localappdata%\Microsoft\Edge\User Data\Default\Cache" (
    rd /s /q "%localappdata%\Microsoft\Edge\User Data\Default\Cache" >nul 2>&1 && echo [10] Edge cache cleared || echo [10] Failed to clear Edge cache
) else (
    echo [10] No Edge cache found. Skipping...
)
...
for /d %%P in ("%localappdata%\Mozilla\Firefox\Profiles\*") do (
    if exist "%%P\cache2" (
        rd /s /q "%%P\cache2" >nul 2>&1 && echo [11] Firefox cache for profile %%P cleared || echo [11] Failed to clear Firefox cache for profile %%P
    ) else (
